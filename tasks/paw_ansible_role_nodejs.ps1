...
if ($env:PT__installdir) {
  $AnsibleDir = Join-Path $env:PT__installdir "lib\puppet_x\ansible_modules\ansible_role_nodejs"
} else {
  # Fallback to Puppet cache directory
  $AnsibleDir = "C:\ProgramData\PuppetLabs\puppet\cache\lib\puppet_x\ansible_modules\ansible_role_nodejs"
}
...
$CollectionPlaybook = Join-Path $AnsibleDir "roles\paw_ansible_role_nodejs\playbook.yml"
...
if ((Test-Path (Join-Path $AnsibleDir "roles")) -and (Test-Path $CollectionPlaybook)) {
  ...
  $PlaybookDir = Join-Path $AnsibleDir "roles\paw_ansible_role_nodejs"
  ...
  $result = @{
    _error = @{
      msg = "playbook.yml not found in $AnsibleDir or $AnsibleDir\roles\paw_ansible_role_nodejs"
...
$ExtraVars = @{}
if ($env:PT_npm_config_prefix) {
  $ExtraVars['npm_config_prefix'] = $env:PT_npm_config_prefix
}
if ($env:PT_nodejs_version) {
  $ExtraVars['nodejs_version'] = $env:PT_nodejs_version
}
if ($env:PT_npm_config_unsafe_perm) {
  $ExtraVars['npm_config_unsafe_perm'] = $env:PT_npm_config_unsafe_perm
}
if ($env:PT_nodejs_npm_global_packages) {
  $ExtraVars['nodejs_npm_global_packages'] = $env:PT_nodejs_npm_global_packages
}
if ($env:PT_nodejs_package_json_path) {
  $ExtraVars['nodejs_package_json_path'] = $env:PT_nodejs_package_json_path
}
if ($env:PT_nodejs_generate_etc_profile) {
  $ExtraVars['nodejs_generate_etc_profile'] = $env:PT_nodejs_generate_etc_profile
}
