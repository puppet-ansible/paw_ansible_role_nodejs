...
# Puppet task for executing Ansible role: ansible_role_nodejs
# This script runs the entire role via ansible-playbook

# Determine the ansible modules directory
if [ -n "$PT__installdir" ]; then
  ANSIBLE_DIR="$PT__installdir/lib/puppet_x/ansible_modules/ansible_role_nodejs"
else
  # Fallback to /opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules
  ANSIBLE_DIR="/opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules/ansible_role_nodejs"
fi
...
if [ -d "$ANSIBLE_DIR/roles" ] && [ -f "$ANSIBLE_DIR/roles/paw_ansible_role_nodejs/playbook.yml" ]; then
  # Collection structure
  PLAYBOOK_PATH="$ANSIBLE_DIR/roles/paw_ansible_role_nodejs/playbook.yml"
  PLAYBOOK_DIR="$ANSIBLE_DIR/roles/paw_ansible_role_nodejs"
  ...
  echo "{\"_error\": {\"msg\": \"playbook.yml not found in $ANSIBLE_DIR or $ANSIBLE_DIR/roles/paw_ansible_role_nodejs\", \"kind\": \"puppet-ansible-converter/playbook-not-found\"}}"
  exit 1
fi
...
EXTRA_VARS="{"
FIRST=true
if [ -n "$PT_npm_config_prefix" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"npm_config_prefix\": \"$PT_npm_config_prefix\""
fi
if [ -n "$PT_nodejs_version" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"nodejs_version\": \"$PT_nodejs_version\""
fi
if [ -n "$PT_npm_config_unsafe_perm" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"npm_config_unsafe_perm\": \"$PT_npm_config_unsafe_perm\""
fi
if [ -n "$PT_nodejs_npm_global_packages" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"nodejs_npm_global_packages\": \"$PT_nodejs_npm_global_packages\""
fi
if [ -n "$PT_nodejs_package_json_path" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"nodejs_package_json_path\": \"$PT_nodejs_package_json_path\""
fi
if [ -n "$PT_nodejs_generate_etc_profile" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"nodejs_generate_etc_profile\": \"$PT_nodejs_generate_etc_profile\""
fi
EXTRA_VARS="$EXTRA_VARS}"
...
if [ $EXIT_CODE -eq 0 ]; then
  echo '{"status": "success", "role": "ansible_role_nodejs"}'
else
  echo "{\"status\": \"failed\", \"role\": \"ansible_role_nodejs\", \"exit_code\": $EXIT_CODE}"
fi
